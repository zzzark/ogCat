...
struct ImDrawList;                  // A single draw command list (generally one per window, conceptually you may see this as a dynamic "static_mesh" builder)
...
namespace ImGui
{
    ...
    IMGUI_API void          ShowMetricsWindow(bool* p_open = NULL);     // create metrics window. display Dear ImGui internals: draw commands (with individual draw calls and _vertices), window list, basic internal state, etc.
    ...
    IMGUI_API bool          IsKeyDown(int user_key_index);                                      // is key being held. == io.KeysDown[user_key_index]. note that imgui doesn't know the semantic of each entry of io.KeysDown[]. Use your own _indices/enums according to how your backend/engine stored them into io.KeysDown[]!
...
// User fill ImGuiIO.KeyMap[] array with _indices into the ImGuiIO.KeysDown[512] array
enum ImGuiKey_
{
    ImGuiKey_Tab,
    ImGuiKey_LeftArrow,
...
struct ImGuiIO
{
    ...
    int         KeyMap[ImGuiKey_COUNT];         // <unset>          // Map of _indices into the KeysDown[512] entries array which represent your "native" keyboard state.
...
struct ImDrawCmd
{
    unsigned int    ElemCount;              // Number of _indices (multiple of 3) to be rendered as triangles. Vertices are stored in the callee ImDrawList's vtx_buffer[] array, _indices in idx_buffer[].
    ImVec4          ClipRect;               // Clipping rectangle (x1, y1, x2, y2). Subtract ImDrawData->DisplayPos to get clipping rectangle in "viewport" coordinates
    ImTextureID     TextureId;              // User-provided texture ID. Set by user in ImfontAtlas::SetTexID() for fonts or passed to Image*() functions. Ignore if never using images or multiple fonts atlas.
    ImDrawCallback  UserCallback;           // If != NULL, call the function instead of rendering the _vertices. clip_rect and texture_id will be set normally.
...
enum ImDrawListFlags_
{
    ...
    ImDrawListFlags_AntiAliasedFill  = 1 << 1   // Filled shapes have anti-aliased edges (*2 the number of _vertices)
};
