...
// Tip: use ImGui::PushID()/PopID() to push _indices or pointers in the ID stack.
...
{
    // FIXME-OPT: we could pre-compute the _indices to fasten this. But only 1 active combo means the waste is limited.
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    // without any carriage return, which would makes ImFont::RenderText() reserve too many _vertices and probably crash. Avoid it altogether.
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel)
    {
        ...
        for (int n = 0; n < 6; n++)
        {
            ...
            // Paint colors over existing _vertices
...
#if 0
    if (hovered && g.HoveredIdNotActiveTimer > 0.50f && bb.GetOriginalW() < tab->WidthContents)
    {
        // Enlarge tab display when hovering
        bb.Max.x = bb.Min.x + (float)(int)ImLerp(bb.GetOriginalW(), tab->WidthContents, ImSaturate((g.HoveredIdNotActiveTimer - 0.40f) * 6.0f));
